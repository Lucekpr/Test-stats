...
## Spis Powszechny (Kingdom Census)

The bot includes an automatic three-channel census system for the server:

- `👥・Mieszkańcy N` — current number of residents.
- `🆕・Ostatnio przybyły NAME` — last member who joined.
- `☠️・Ostatnia kara śmierci NAME` — last user who was banned.

Configure it in `.env` using `CENSUS_GUILD_ID`, `CENSUS_RESIDENT_ROLE_ID`, `CENSUS_MEMBERS_CHANNEL_ID`, `CENSUS_LAST_JOIN_CHANNEL_ID`, and `CENSUS_LAST_BAN_CHANNEL_ID`. The join/ban records are stored in the bot database, so they survive restarts when PostgreSQL is available.

The bot requires the Discord `Guild Members` and `Guild Bans` intents, which are already enabled in `src/app.js`. The bot also needs `Manage Channels` permission so it can rename the three census channels.
...
The server Census can be configured from Discord with `/serverstats census`. See `CENSUS-SETUP.md` for the interactive configuration options.
