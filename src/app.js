...
import { getServerCounters, saveServerCounters, updateCounter, updateCensusChannels } from './services/serverstatsService.js';
...
class TitanBot extends Client {
  ...
  async start() {
    try {
      ...
      // Perform an immediate census/counter sync so configured channels are correct
      // right after startup instead of waiting for the first 15-minute cron run.
      try {
        await this.updateAllCounters();
        startupLog('Initial counters/census sync complete');
      } catch (error) {
        logger.warn('Initial counters/census sync failed:', error.message);
      }
  ...
  async updateAllCounters() {
    ...
    for (const [guildId, guild] of this.guilds.cache) {
      try {
        ...
        try {
          await updateCensusChannels(this, guild);
        } catch (censusError) {
          logger.error(`Error updating census for guild ${guildId}:`, censusError);
        }
      } catch (error) {
