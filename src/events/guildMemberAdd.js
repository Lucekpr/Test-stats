...
import { getServerCounters, updateCounter, recordCensusJoin } from '../services/serverstatsService.js';
...
export default {
  ...
  async execute(member) {
    try {
        ...
        try {
            await recordCensusJoin(member.client, member);
        } catch (error) {
            logger.debug('Error updating census on member join:', error);
        }
